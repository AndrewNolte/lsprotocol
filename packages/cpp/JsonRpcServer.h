...
protected:
    ...
    std::variant<rfl::Generic, RpcError, std::nullopt_t> processMessage(RpcRequest request) {
        if (!request.id) {
            ...
            if (it != notifications.end()) {
                ...
                try {
                    ...
                    std::cerr << "-/-> " << request.method << " Error: " << e.what() << std::endl;
                }
            }
            else if (request.method.find("$/") == 0) {
                std::cerr << "<-/- " << request.method << " (ignoring threaded req)" << std::endl;
            }
            else {
                std::cerr << "<-/- " << request.method << " (method not found)" << std::endl;
            }
            return std::nullopt;
        }
        ...
        if (it != requests.end()) {
            try {
                std::cerr << "<--- " << request.method << " " << id << std::endl;
            ...
            std::cerr << "<-/- " << request.method << " (not found)" << std::endl;
        }
