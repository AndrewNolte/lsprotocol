...
class Server {
    ...
    /// workspace before the files are created. Hence the `WorkspaceEdit` can
    /// not manipulate the content of the file to be created.
    ///
    /// @since 3.16.0
    ...
    /// Clients
    ///  need to advertise support for WorkspaceSymbols via the client
    ...
    /// TextDocumentPositionParams} the response is of type {@link Definition}
    ...
    /// type
    /// {@link InlineCompletion InlineCompletion[]} or a Thenable that resolves
    /// to such.
    ///
    /// @since 3.18.0
    /// @proposed
    ...
    /// response is of type {@link SymbolInformation SymbolInformation[]} or a
    /// Thenable that resolves to such.
    std::variant<std::vector<SymbolInformation>,
    ...
    /// request's parameter is of type {@link DocumentColorParams} the response
    /// is of type {@link ColorInformation ColorInformation[]} or a Thenable
    /// that resolves to such.
    ...
    /// is of type {@link CompletionItem CompletionItem[]} or {@link
    /// CompletionList} or a Thenable that resolves to such.
    ///
    /// The request can delay the computation of the {@link
    /// CompletionItem.detail `detail`} and {@link CompletionItem.documentation
    /// `documentation`} properties to the `completionItem/resolve` request.
    /// However, properties that are needed for the initial sorting and
    /// filtering, like `sortText`, `filterText`, `insertText`, and `textEdit`,
    ...
    /// response is of type {@link CompletionItem} or a Thenable that resolves
    /// to such.
